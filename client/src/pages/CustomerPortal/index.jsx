    ...
    <div className="modal open" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-content" style={{ maxWidth: 500 }}>
        ...
        <div className="modal-body">
          <p style={{ marginBottom: '1.2rem', color: 'var(--text-muted)', fontSize: '0.92rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            {Object.values(CERTIFICATE_TYPES).map(ct => (
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.border = `1.5px solid ${ct.color}cc`; e.currentTarget.style.transform = 'translateX(4px)'; }}
                onMouseLeave={e => { e.currentTarget.style.border = `1.5px solid ${ct.color}44`; e.currentTarget.style.transform = 'translateX(0)'; }}
              >
                <div style={{ width: 46, height: 46, borderRadius: '50%', background: ct.color + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <i className={ct.icon} style={{ color: ct.color, fontSize: '1.2rem' }}></i>
                </div>
                <div>
                  <div style={{ fontWeight: 700, fontSize: '1.05rem', color: ct.color }}>{ct.label}</div>
                  <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '2px' }}>{ct.sublabel}</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
                    {ct.documents.slice(0, 2).map((d, i) => (
                      <span key={i} style={{ fontSize: '0.72rem', background: 'rgba(255,255,255,0.08)', borderRadius: '4px', padding: '1px 6px', color: 'var(--text-muted)' }}>
                        {d}
                      </span>
                    ))}
                    {ct.documents.length > 2 && <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>+{ct.documents.length - 2} more</span>}
                  </div>
                </div>
                <i className="fa-solid fa-chevron-right" style={{ marginLeft: 'auto', color: ct.color, opacity: 0.7 }}></i>
              </button>
            ))}
          </div>
        </div>
...
function CertificateFormModal({ certType, service, onClose, showToast, onSubmitSuccess, prefilledName, prefilledPhone, uploadToken }) {
  const [formValues, setFormValues] = useState(() => {
    ...
    if (prefilledPhone) init['mobile'] = prefilledPhone;
    return init;
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!aadharFile) { showToast('कृपया आधार कार्ड अपलोड करें। / Please upload Aadhar Card.', 'error'); return; }
    if (!photoFile) { showToast('कृपया फोटो अपलोड करें। / Please upload Passport Photo.', 'error'); return; }
    setIsSubmitting(true);
    const formData = new FormData();
    formData.append('clientName', formValues.applicantName || '');
    formData.append('clientPhone', formValues.mobile || '');
    formData.append('serviceType', `${service.id}_${certType.id}`);
    formData.append('serviceName', `${certType.label} (${certType.sublabel})`);

    // Compose notes from all filled fields
      ...
      <div className="modal-content" style={{ maxWidth: 620 }}>
        <div className="modal-header" style={{ borderBottom: `3px solid ${certType.color}` }}>
          <h3 style={{ color: certType.color }}>
            ...
            <span style={{ fontSize: '0.8rem', fontWeight: 400, color: 'var(--text-muted)', marginLeft: 8 }}>{certType.sublabel}</span>
          </h3>
          <button className="modal-close" onClick={onClose}>&times;</button>
        </div>
        <div className="modal-body">
          {uploadToken && (
            <div style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: '8px', padding: '0.75rem 1rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.6rem', fontSize: '0.88rem', color: '#4ade80' }}>
              ...
              <span>This form was pre-loaded from your <strong>WhatsApp conversation</strong>.</span>
            </div>
          )}

          {/* Required Documents Info Box */}
          <div className="requirements-box" style={{ borderLeft: `4px solid ${certType.color}` }}>
            <h4><i className="fa-solid fa-circle-info"></i> आवश्यक दस्तावेज़ / Required Documents:</h4>
            <ul>
              {certType.documents.map((d, i) => <li key={i}>{d}</li>)}
            </ul>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-grid-2">
              {certType.fields.map(field => (
                <div className="form-group" key={field.name} style={field.name === 'thana' || field.name === 'jaati' || field.name === 'upjaati' ? {} : {}}>
                  <label>
                    {field.label}
                    {field.required && <span style={{ color: certType.color, marginLeft: 3 }}>*</span>}
                    {field.fixed && <span style={{ fontSize: '0.75rem', marginLeft: 6, color: 'var(--text-muted)', fontStyle: 'italic' }}>(fixed)</span>}
                  </label>
                  {field.type === 'select' ? (
                    <select
                      ...
                      style={{ padding: '0.8rem', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--bg-secondary)', color: 'var(--text-color)', width: '100%' }}
                    >
                      <option value="" disabled>Select {field.label.split('/')[0].trim()}</option>
            ...
            <div className="form-group" style={{ marginTop: '1rem' }}>
              <label>अनिवार्य दस्तावेज़ / Mandatory Documents <span style={{ color: certType.color }}>*</span></label>
              <div style={{ display: 'flex', gap: '1rem', flexDirection: 'column', marginTop: '0.5rem' }}>
                <div style={{ background: 'var(--bg-tertiary)', padding: '0.8rem', borderRadius: '8px', border: `1px solid ${aadharFile ? certType.color : 'var(--border-color)'}`, display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: '0.9rem' }}><i className="fa-solid fa-address-card" style={{ marginRight: 6 }}></i>आधार कार्ड / Aadhar Card</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Required (Image or PDF)</div>
                  </div>
                  <input type="file" accept="image/*,application/pdf" onChange={e => setAadharFile(e.target.files[0])} style={{ maxWidth: '100%', fontSize: '0.8rem' }} />
                </div>
                <div style={{ background: 'var(--bg-tertiary)', padding: '0.8rem', borderRadius: '8px', border: `1px solid ${photoFile ? certType.color : 'var(--border-color)'}`, display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: '0.9rem' }}><i className="fa-solid fa-camera" style={{ marginRight: 6 }}></i>पासपोर्ट साइज फोटो / Passport Photo</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Required (Image Only)</div>
                  </div>
                  <input type="file" accept="image/*" onChange={e => setPhotoFile(e.target.files[0])} style={{ maxWidth: '100%', fontSize: '0.8rem' }} />
                </div>
              </div>
            </div>
            ...
            <div className="form-group" style={{ marginTop: '1rem' }}>
              <label>
                ...
                <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginLeft: 6 }}>(Max 10 files)</span>
              </label>
              <div
            ...
            <div className="modal-footer">
              ...
              <button type="submit" className="btn btn-primary" disabled={isSubmitting}
                style={{ background: certType.color, borderColor: certType.color }}>
                {isSubmitting
...
export default function CustomerPortal({ shopSettings, showToast, adminToken, onSubmitSuccess }) {
  ...
  const handleCertSelect = (certType) => {
    setShowCertPicker(false);
